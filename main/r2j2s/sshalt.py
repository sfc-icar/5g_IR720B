...
class ssh:
    def __init__(self):
        ...
        self.CMD2 = 'sudo -S qmicli -d /dev/cdc-wdm1 --nas-get-lte-cphy-ca-info'
        self.CMD3 = 'sudo -S qmicli -d /dev/cdc-wdm1 --nas-get-cell-location-info'
    ...
    def ssh_cli(self):
        # ３つ目
        stdin, stdout, stderr = client.exec_command(self.CMD3)
        stdin.write('admin\n')
        stdin.flush()
        cmd_result = ''
        for line in stdout:
    ...
    def ssh_cpca(self):
        # ２つ目
        stdin, stdout, stderr = client.exec_command(self.CMD2)
        stdin.write('admin\n')
        stdin.flush()
        cmd_result = ''
        for line in stdout:
